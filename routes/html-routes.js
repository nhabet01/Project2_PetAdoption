...
router.post("/login", function(req, res) {
    ...
    db.User.findOne({
        ...
        if (data) {
            ...
            res.render('login', { wrongData: 'User does not exist!' })
...
router.post('/search/:username', (req, res) => {
    ...
    if (req.session.logged_in && req.session.user_name == req.params.username) {
        ...
        console.log('THIS IS newZIP ' + ' ' + newzip)
...
router.post("/favAnimals", function(req, res) {
    ...
    db.Favorites.findAll({
        ...
        if(data.length >= 7){
            console.log("Max favorites reached")
            res.redirect(`/foundAnimals/${usrname}`)
            //maybe render instead and let user know to limit reached?
        }else {
